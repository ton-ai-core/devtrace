...
export function installStackLogger({
  ...
  const orig: Record<string, LogFn> = {};
  ...
  for (const m of methods) {
    orig[m] = console[m] as LogFn;
    (console as unknown as Record<string, LogFn>)[m] = (...args: unknown[]): void => {
      ...
      void (async (): Promise<void> => {
        ...
        const toUrl = (file?: string): string | undefined => {
          ...
          // Remove import.meta.env reference for standalone module
          return file;
        };
